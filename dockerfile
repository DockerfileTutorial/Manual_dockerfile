# Run command 'docker build -f ../dockerfile -t 123 .'
FROM mypython:3.8-alpine-changed-url as opencv_builder
LABEL maintainer "Yiting"

COPY . /work
WORKDIR /work/

env OpenCV_install_dir=/usr/local
...
RUN apk add --update --no-cache build-base linux-headers cmake \
	...
	&& cmake -DCMAKE_BUILD_TYPE=RELEASE \
		 ...
		 -DPYTHON3_LIBRARY=/usr/local/lib/libpython3.so \
...
ENV LD_LIBRARY_PATH=$LD_LIBRARY_PATH:"$OpenCV_install_dir"/lib64

# build darknet 
WORKDIR /work/darknet
RUN make \
    && ./darknet \
    && apk del build-base opencv_deps
